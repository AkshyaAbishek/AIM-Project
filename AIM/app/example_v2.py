...
class AIMDemoGUI:
    ...
    def setup_ui(self):
        ...
        self.create_button(buttons_frame, "📊 Life Field Mapping", lambda: self.show_field_mapping_with_loading("life"), 0, 2, "#e67e22", "#d35400")
        ...
        self.create_button(buttons_frame, "📊 Annuity Field Mapping", lambda: self.show_field_mapping_with_loading("annuity"), 1, 2, "#8e44ad", "#7d3c98")
...
Ensure all string values are in quotes and numbers are without quotes."""
        ...
        def copy_sample():
            ...
            messagebox.showinfo("Copied to Clipboard", "Example data has been copied to clipboard!")
        ...
        tk.Button(button_frame, text="📋 Copy Sample", command=copy_sample, 
                 bg="#9b59b6", fg="white", font=("Arial", 11), relief="raised", bd=2).pack(side="left", padx=8)
    ...
    def show_field_mapping(self, product_type):
        ...
        self.clear_results()
        self.log_result(f"🔄 Loading field mapping for {product_type.title()}...")
        self.root.update()  # Force UI update
        
        try:
            import pandas as pd
        except ImportError:
            self.log_result("❌ Failed to load field mapping: pandas module missing")
        ...
        self.log_result("✅ Field mapping dialog opening...")
        self.root.update()  # Force UI update
        ...
        def create_mapping():
            ...
            self.clear_results()
            self.log_result("🔄 Starting Excel field mapping creation...")
            self.log_result("⏳ Please wait while the mapping is generated...")
            self.root.update()  # Force UI update
            
            dialog.destroy()
            # Use after_idle to ensure UI updates before starting the heavy operation
            self.root.after_idle(lambda: self.create_excel_mapping(product_type.lower(), output_path, calculator_path))
    ...
    def process_bulk_upload(self, upload_path, parent_dialog):
        ...
        try:
            ...
            self.clear_results()
            self.log_result("🔄 Starting bulk data upload process...")
            self.log_result("📂 Reading Excel file...")
            self.root.update()  # Force UI update
            ...
            if df.empty:
                self.log_result("❌ Upload failed: Excel file is empty")
                messagebox.showerror("Error", "Excel file is empty")
                return
            
            self.log_result(f"✅ Excel file loaded successfully - {len(df)} rows found")
            parent_dialog.destroy()
            
            self.log_result("🔄 Processing and validating data...")
            self.root.update()
            
            # Process and preview data first
            processed_data = []
            error_count = 0
            ...
            self.show_bulk_preview_dialog(processed_data, error_count, upload_path)
    ...
    def show_bulk_preview_dialog(self, processed_data, error_count, upload_path):
        """Show preview of bulk data with option to save to database."""
        dialog = tk.Toplevel(self.root)
        dialog.title("📦 Bulk Data Preview")
        ...
        tk.Label(header_frame, text="📦 Bulk Data Preview & Save Options", 
                font=("Segoe UI", 14, "bold"), fg="#ecf0f1", bg="#9b59b6").pack(expand=True)
        
        # Scroll hint
        scroll_hint = tk.Label(header_frame, text="💡 Tip: Scroll down to see save options", 
                              font=("Segoe UI", 9), fg="#ecf0f1", bg="#9b59b6")
        scroll_hint.pack(pady=(0, 5))
        
        # Create main scrollable area
        main_canvas = tk.Canvas(dialog, bg="#f8f9fa", highlightthickness=0)
        ...
        tk.Label(summary_frame, text=f"📊 Preview Summary", 
                font=("Segoe UI", 12, "bold"), fg="#2e7d32", bg="#e8f5e8").pack(pady=10)
        
        tk.Label(summary_frame, text=f"✅ Valid records found: {len(processed_data)}", 
                font=("Segoe UI", 10), fg="#2e7d32", bg="#e8f5e8").pack(pady=2)
        
        if error_count > 0:
            tk.Label(summary_frame, text=f"❌ Rows with errors: {error_count}", 
                    font=("Segoe UI", 10), fg="#d32f2f", bg="#e8f5e8").pack(pady=2)
        
        tk.Label(summary_frame, text=f"📁 Source file: {upload_path.split('/')[-1]}", 
                font=("Segoe UI", 9), fg="#666666", bg="#e8f5e8").pack(pady=2)
        
        # Preview data
        preview_frame = tk.LabelFrame(content_frame, text="📋 Data Preview", 
                                    font=("Arial", 11, "bold"), bg="#e3f2fd", fg="#1565c0")
        preview_frame.pack(fill="x", pady=(0, 15))
        
        preview_text = scrolledtext.ScrolledText(preview_frame, height=15, width=100, 
                                               font=("Consolas", 9), bg="#ffffff", fg="#2c3e50",
                                               relief="solid", bd=1)
        preview_text.pack(padx=15, pady=10, fill="x")
        
        # Display preview data
        for i, record in enumerate(processed_data[:10], 1):  # Show first 10 records
            preview_text.insert(tk.END, f"\n--- Record {record['row_number']} (Product: {record['product_type']}) ---\n")
            preview_text.insert(tk.END, json.dumps(record['data'], indent=2))
            preview_text.insert(tk.END, "\n" + "-" * 60 + "\n")
        
        if len(processed_data) > 10:
            preview_text.insert(tk.END, f"\n... and {len(processed_data) - 10} more records ...\n")
        
        # Instructions frame  
        instructions_frame = tk.LabelFrame(content_frame, text="📋 What's Next?", 
                                         font=("Arial", 11, "bold"), bg="#e8f5e8", fg="#2e7d32")
        instructions_frame.pack(fill="x", pady=(0, 15))
        
        instructions_content = tk.Frame(instructions_frame, bg="#e8f5e8")
        instructions_content.pack(pady=15, padx=15)
        
        tk.Label(instructions_content, text="✅ Data has been successfully processed and is ready for action:", 
                font=("Segoe UI", 10, "bold"), fg="#2e7d32", bg="#e8f5e8").pack(anchor="w", pady=(0, 5))
        
        tk.Label(instructions_content, text="• Use 'Save to Database' to permanently store all records", 
                font=("Segoe UI", 9), fg="#2e7d32", bg="#e8f5e8").pack(anchor="w", pady=2, padx=20)
        
        tk.Label(instructions_content, text="• Use 'Close Preview' to exit without saving anything", 
                font=("Segoe UI", 9), fg="#2e7d32", bg="#e8f5e8").pack(anchor="w", pady=2, padx=20)
        ...
        def save_to_database():
            result = messagebox.askyesno("Confirm Save", 
                                       f"Are you sure you want to save all {len(processed_data)} records to the database?\n\n"
                                       f"This action will:\n"
                                       f"• Add new records to the database\n"
                                       f"• Skip any duplicate records\n"
                                       f"• Show a detailed summary when complete\n\n"
                                       f"Continue with save?")
            if result:
                main_canvas.unbind_all("<MouseWheel>")
                self.save_bulk_data_to_database(processed_data, dialog)
        
        # Close without saving
        def close_preview():
            result = messagebox.askyesno("Close Preview", 
                                       f"Close preview without saving?\n\n"
                                       f"All {len(processed_data)} processed records will be discarded.\n"
                                       f"No data will be saved to the database.\n\n"
                                       f"Are you sure?")
            if result:
                main_canvas.unbind_all("<MouseWheel>")
                dialog.destroy()
                self.log_result("👀 Bulk data preview closed - no data saved")
                messagebox.showinfo("Preview Closed", 
                                   f"Preview session ended.\n\n"
                                   f"Records processed: {len(processed_data)}\n"
                                   f"No data was saved to the database.")
        
        # Buttons with clear labels and colors
        tk.Button(button_frame, text="💾 Save to Database", command=save_to_database,
                 bg="#27ae60", fg="white", font=("Arial", 12, "bold"), relief="raised", bd=3,
                 width=18, height=2).pack(side="left", padx=15)
        
        tk.Button(button_frame, text="👀 Close Preview", command=close_preview,
                 bg="#95a5a6", fg="white", font=("Arial", 12, "bold"), relief="raised", bd=3,
                 width=18, height=2).pack(side="left", padx=15)
        
        # Cleanup function to unbind mousewheel when dialog is closed
        def on_dialog_close():
            main_canvas.unbind_all("<MouseWheel>")
            dialog.destroy()
    ...
    def save_bulk_data_to_database(self, processed_data, parent_dialog):
        """Save the bulk processed data to database."""
        parent_dialog.destroy()
        
        self.clear_results()
        self.log_result("💾 Saving bulk data to database...")
        ...
        for record in processed_data:
            try:
                success, message = self.save_data_to_db(record['data'], record['product_type'])
                if success:
                    success_count += 1
                else:
                    duplicate_count += 1
                ...
                error_count += 1
                self.log_result(f"⚠️ Error saving row {record['row_number']}: {e}")
        ...
        # Show results
        self.log_result(f"✅ Bulk save completed!")
        self.log_result(f"📊 Successfully saved: {success_count} records")
        self.log_result(f"📊 Duplicates skipped: {duplicate_count} records")
        self.log_result(f"📊 Errors encountered: {error_count} records")
        
        messagebox.showinfo("Bulk Save Complete", 
                           f"Bulk data save completed!\n\n"
                           f"✅ Saved: {success_count}\n"
                           f"⚠️ Duplicates: {duplicate_count}\n"
                           f"❌ Errors: {error_count}\n\n"
                           f"Check the results area for detailed logs.")
    def create_excel_mapping(self, product_type, output_path, calculator_path):
        ...
        try:
            ...
            self.log_result("📋 Initializing field mapping process...")
            self.root.update()  # Force UI update
            
            # Get FAST UI fields from the processor
            self.log_result("🔍 Retrieving FAST UI field mappings...")
            self.root.update()
            try:
                ...
                if not mappings:
                    ...
                    self.log_result(f"✅ Found {len(mappings)} FAST UI fields")
            except Exception as e:
                self.log_result(f"⚠️ Could not get mappings from processor: {e}")
                self.log_result("📋 Using sample field mappings instead...")
                # Use sample mappings
                mappings = {
                    "applicant_first_name": "John",
                    "applicant_last_name": "Doe", 
            ...
            self.log_result("📊 Reading actuarial calculator Excel file...")
            self.root.update()
            calculator_fields = []
            try:
            ...
            # Create mapping data
            self.log_result("🔗 Creating field mapping suggestions...")
            self.root.update()
            mapping_data = []
            ...
            self.log_result(f"📝 Generated {len(mapping_data)} field mappings")
            self.root.update()
            
            # Create Excel file
            self.log_result("💾 Creating Excel mapping file...")
            self.root.update()
    ...
    def show_field_mapping_with_loading(self, product_type):
        """Wrapper function to show loading message before opening field mapping."""
        # Show immediate loading message
        self.clear_results()
        self.log_result(f"🔄 Preparing {product_type.title()} Field Mapping...")
        self.log_result("⏳ Please wait while the system initializes...")
        self.root.update()  # Force UI update
        
        # Use after_idle to ensure UI updates before starting the heavy operation
        self.root.after_idle(lambda: self.show_field_mapping(product_type))
