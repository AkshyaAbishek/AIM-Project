...
def compare():
    ...
    try:
        print("Step 1: Calling db_manager.get_all_data()...")
        all_data = db_manager.get_all_data()
        print(f"Step 2: get_all_data() returned {len(all_data)} records")
        
        if all_data:
            print("Step 3: Sample records:")
            for i, record in enumerate(all_data[:3]):
                print(f"  Record {i+1}: {dict(record)}")
        else:
            print("Step 3: No records returned from get_all_data()")
            
            # Let's try a direct database query to see what's there
            print("Step 4: Trying direct database query...")
            with sqlite3.connect(db_manager.db_path) as conn:
                cursor = conn.cursor()
                cursor.execute("SELECT COUNT(*) FROM user_data")
                count = cursor.fetchone()[0]
                print(f"Direct query: Found {count} total records in database")
                
                if count > 0:
                    cursor.execute("SELECT id, name, product_type, validation_status FROM user_data LIMIT 5")
                    sample_records = cursor.fetchall()
                    print("Sample records from direct query:")
                    for record in sample_records:
                        print(f"  {record}")
        
        # Pass the same data format as view_data but rename for template compatibility
        processed_records = all_data
        print(f"Step 5: Setting processed_records to {len(processed_records)} records")
