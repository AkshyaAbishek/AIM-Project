...
def compare():
    ...
    try:
        ...
        with sqlite3.connect(db_manager.db_path) as conn:
            ...
            print(f"DIRECT SQL COUNT: {direct_count} records in user_data table")
            
            if direct_count > 0:
                cursor.execute("SELECT id, name, product_type FROM user_data LIMIT 3")
                sample_records = cursor.fetchall()
                print(f"DIRECT SQL SAMPLE: {[dict(r) for r in sample_records]}")
    except Exception as sql_error:
        print(f"DIRECT SQL ERROR: {sql_error}")
    
    # Now test the db_manager method
    try:
        all_data = db_manager.get_all_data()
        print(f"DB_MANAGER.get_all_data() returned: {type(all_data)} with {len(all_data)} records")
        
        if all_data:
            print(f"First record type: {type(all_data[0])}")
            print(f"First record: {dict(all_data[0]) if hasattr(all_data[0], 'keys') else all_data[0]}")
        
        processed_records = all_data
        print(f"Final processed_records for template: {len(processed_records)} records")
        
    except Exception as manager_error:
        print(f"DB_MANAGER ERROR: {manager_error}")
        import traceback
        traceback.print_exc()
        processed_records = []
...
@app.route('/debug/data-comparison')
def debug_data_comparison():
    """Debug endpoint to compare what view_data vs compare gets"""
    try:
        # Method 1: What view_data page gets
        view_data_records = db_manager.get_all_data()
        
        # Method 2: What compare page was trying to get
        compare_records = []
        with sqlite3.connect(db_manager.db_path) as conn:
            conn.row_factory = sqlite3.Row
            cursor = conn.cursor()
            cursor.execute("""
                SELECT id, name, product_type, created_date, calculator_path, json_data, validation_status
                FROM user_data 
        ...
        debug_info = {
            'view_data_method': {
                ...
                'records': [
                    ...
                    } for r in view_data_records[:5]
                ]
            },
            'compare_method': {
                'count': len(compare_records),
                'records': [
                    {
                        'id': r.get('id'),
                        'name': r.get('name'),
...
@app.route('/debug')
def debug_page():
    """Simple debug page to show data availability"""
    try:
        # Get data using view_data method
        view_data_records = db_manager.get_all_data()
        ...
        with sqlite3.connect(db_manager.db_path) as conn:
            conn.row_factory = sqlite3.Row
            cursor = conn.cursor()
            cursor.execute("SELECT * FROM user_data ORDER BY created_date DESC LIMIT 10")
        ...
        <!DOCTYPE html>
        <html>
        <head><title>AIM Debug Information</title>
        <style>
        ...
        <body>
        <h1>AIM Debug Information</h1>
        
        <div class="section {'good' if view_data_records else 'bad'}">
            <h2>View Data Method Results</h2>
            <p><strong>Records found:</strong> {len(view_data_records)}</p>
            {"<p>✅ This method is working!</p>" if view_data_records else "<p>❌ No records found</p>"}
            <h3>Records:</h3>
            <pre>{json.dumps([{{
                'id': r.get('id'),
                'name': r.get('name'),
            ...
            }} for r in view_data_records[:3]], indent=2)}</pre>
        </div>
        
        <div class="section {'good' if raw_records else 'bad'}">
            <h2>Raw Database Records</h2>
            <p><strong>Records found:</strong> {len(raw_records)}</p>
            <h3>Sample records (first 3):</h3>
            <pre>{json.dumps([dict(r) for r in raw_records[:3]], indent=2, default=str)}</pre>
        </div>
        
        <div class="section">
            <h2>Database Info</h2>
            <p><strong>Database path:</strong> {db_manager.db_path}</p>
            <p><strong>File exists:</strong> {os.path.exists(db_manager.db_path)}</p>
        </div>
        
        <div class="section">
            <h2>Quick Actions</h2>
            <p><a href="/view-data">View Data Page</a> | <a href="/compare">Compare Page</a> | <a href="/upload">Upload Data</a></p>
        </div>
        ...
        return f"<h1>Debug Error</h1><pre>{str(e)}\n\n{traceback.format_exc()}</pre>"

@app.route('/debug/database')
def debug_database():
    """Debug route to check database contents"""
    try:
        print("=== DEBUG DATABASE ROUTE ===")
        
        # Get raw database info
        db_exists = os.path.exists(db_manager.db_path)
        db_size = os.path.getsize(db_manager.db_path) if db_exists else 0
        ...
        if db_exists:
            ...
            with sqlite3.connect(db_manager.db_path) as conn:
                ...
                if 'user_data' in tables:
                    ...
                    debug_info['total_records'] = count
                    ...
                    if count > 0:
                        ...
                        cursor.execute("""
                            ...
                            FROM user_data ORDER BY created_date DESC LIMIT 10
                        """)
                        records = cursor.fetchall()
                        debug_info['sample_records'] = [dict(record) for record in records]
                        
                        # Test the exact same call as our routes
                        test_data = db_manager.get_all_data()
                        debug_info['get_all_data_result'] = {
                            'count': len(test_data),
                            'type': str(type(test_data)),
                            'sample': [dict(record) for record in test_data[:3]] if test_data else []
                        }
                else:
                    debug_info['error'] = 'user_data table not found'
        else:
            debug_info['error'] = 'Database file does not exist'
...
@app.route('/debug/compare-vs-viewdata')
def debug_compare_vs_viewdata():
    """Debug route to compare the exact same logic as both routes"""
    debug_info = {
    ...
    try:
        # Test 1: Direct SQL (what I added to compare route)
        with sqlite3.connect(db_manager.db_path) as conn:
            conn.row_factory = sqlite3.Row
            cursor = conn.cursor()
            cursor.execute("SELECT COUNT(*) FROM user_data")
            debug_info['direct_sql_count'] = cursor.fetchone()[0]
            
            if debug_info['direct_sql_count'] > 0:
                cursor.execute("SELECT id, name, product_type, session_id FROM user_data LIMIT 5")
                debug_info['direct_sql_sample'] = [dict(r) for r in cursor.fetchall()]
        
        # Test 2: Exact same call as view_data route
        view_data_result = db_manager.get_all_data()
        debug_info['view_data_method'] = {
            'count': len(view_data_result),
            'type': str(type(view_data_result)),
            'sample': [dict(r) for r in view_data_result[:3]] if view_data_result else []
        }
        
        # Test 3: Same call but assign to processed_records (like compare route)
        processed_records = db_manager.get_all_data()
        debug_info['compare_method'] = {
            'count': len(processed_records),
            'type': str(type(processed_records)),
            'sample': [dict(r) for r in processed_records[:3]] if processed_records else []
        }
        
        # Test 4: Check if there are session issues
        from flask import session
        debug_info['flask_session'] = dict(session) if session else {}
...
if __name__ == '__main__':
    ...
    # Debug: Detailed compare vs view_data investigation
