...
def compare():
    ...
    try:
        # Use EXACTLY the same method as view_data page
        all_data = db_manager.get_all_data()
        
        print(f"Compare page: Found {len(all_data)} records using view_data method")
        if all_data:
            print("Records from view_data method:")
            for i, record in enumerate(all_data[:5]):  # Show first 5
                print(f"  {i+1}. ID: {record.get('id')}, Name: {record.get('name', 'Unknown')}, Status: {record.get('status', 'Unknown')}")
        
        # Convert to format needed for compare page dropdown
        processed_records = []
        if all_data:
            with sqlite3.connect(db_manager.db_path) as conn:
                conn.row_factory = sqlite3.Row
                ...
                for record in all_data:
                    # Get the json_data field which is needed for comparison
                    cursor.execute("""
                    ...
                    if full_record:
                        processed_records.append(dict(full_record))
        
        print(f"Compare page: Prepared {len(processed_records)} records for dropdown")
...
if __name__ == '__main__':
    ...
    # Debug: Force restart to test process data status update fix
