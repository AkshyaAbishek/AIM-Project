...
def api_upload_json():
    """API endpoint to upload JSON data from compare page"""
    try:
        data = request.get_json()
        name = data.get('name')
        product_type = data.get('product_type')
        ...
        if not name or not product_type or not json_data:
        ...
        result = db_manager.store_data(
            ...
            json_data=json_data,
            session_id=session['session_id'],
            file_source='Direct Upload'
        )
        
        if result['success']:
            return jsonify({'success': True, 'message': 'Data uploaded successfully', 'record_id': result['record_id']})
        else:
            return jsonify({'success': False, 'message': result['message']})
            
    except Exception as e:
        print(f"Error in JSON upload: {e}")
