        ...
        with sqlite3.connect(self.db_path) as conn:
            conn.executescript("""
                CREATE TABLE IF NOT EXISTS user_data (
                    ...
                    name TEXT NOT NULL,
                    product_type TEXT NOT NULL CHECK(product_type IN ('life_insurance', 'annuity', 'health_insurance', 'property_casualty', 'disability')),
            ...
            try:
                conn.execute("ALTER TABLE user_data ADD COLUMN calculator_path TEXT")
            except sqlite3.OperationalError:
                pass  # Column already exists
            
            try:
                conn.execute("ALTER TABLE user_data ADD COLUMN raw_data TEXT")
            except sqlite3.OperationalError:
                pass
            
            try:
                conn.execute("ALTER TABLE user_data ADD COLUMN processed_data TEXT")
            except sqlite3.OperationalError:
                pass
            
            try:
                conn.execute("ALTER TABLE user_data ADD COLUMN updated_date TIMESTAMP DEFAULT CURRENT_TIMESTAMP")
            except sqlite3.OperationalError:
                pass
            
            try:
                conn.execute("ALTER TABLE user_data ADD COLUMN status TEXT DEFAULT 'pending'")
            except sqlite3.OperationalError:
                pass
            
            try:
                conn.execute("ALTER TABLE user_data ADD COLUMN source_file TEXT")
            except sqlite3.OperationalError:
                pass
            
            try:
                conn.execute("ALTER TABLE user_data ADD COLUMN notes TEXT")
            except sqlite3.OperationalError:
                pass
...
def load_calculator_reference(calculator_path: str, product_type: str) -> Dict:
...
def perform_data_comparison(source_data: Dict, calculator_data: Dict) -> Dict:
...
def api_compare_data():
    ...
    try:
        ...
        calculator_data = load_calculator_reference(calculator_path, record['product_type'])
        
        # Perform comparison
        comparison_result = perform_data_comparison(source_data, calculator_data)
