...
def index():
    ...
    try:
        ...
        # Create a simple response to test if templates work
        return f"""
        <!DOCTYPE html>
        <html>
        <head><title>AIM Test</title></head>
        <body>
        <h1>AIM Dashboard Test</h1>
        <p>Total Records: {stats['total_records']}</p>
        <p>Recent Data: {len(recent_data)} items</p>
        <p>Session ID: {session['session_id']}</p>
        </body>
        </html>
        """
        
        # Original template rendering (commented out for debugging)
