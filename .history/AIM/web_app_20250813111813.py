...
def compare():
    ...
    with sqlite3.connect(db_manager.db_path) as conn:
        ...
        cursor.execute("""
            ...
            FROM user_data 
            WHERE status IN ('processed', 'saved') OR validation_status = 'processed'
    ...
    return render_template('compare.html', processed_records=processed_records, available_calculators=available_calculators)
...
def api_compare_data():
    """API endpoint to compare source data with calculator template"""
    try:
        data = request.get_json()
        ...
        if isinstance(record.get('json_data'), str):
            try:
                source_data = json.loads(record['json_data'])
            except (json.JSONDecodeError, TypeError):
                source_data = {}
        else:
            source_data = record.get('json_data', {})
        
        # Load calculator configuration
        calculator_config = db_manager.load_calculator_configuration(calculator_path, record.get('product_type', ''))
        
        # Perform comparison
        comparison_result = db_manager.perform_data_comparison(source_data, calculator_config)
        
        # Generate Excel file
        excel_path = db_manager.generate_comparison_excel(comparison_result, record)
        ...
        return jsonify({
            ...
            'excel_file': {
                'filename': filename,
                'download_url': url_for('download_comparison_excel', filename=filename)
            }
        })
...
def api_export_comparison_excel():
    """API endpoint to export comparison results as Excel file"""
    try:
        data = request.get_json()
        ...
        if not comparison_data or not record_info:
            return jsonify({
                'success': False,
                'message': 'Comparison data and record info are required'
            }), 400
        
        # Generate Excel file
        excel_path = db_manager.generate_comparison_excel(comparison_data, record_info)
        ...
        return jsonify({
            ...
            'download_url': url_for('download_comparison_excel', filename=os.path.basename(excel_path))
        })
...
@app.route('/download/comparison/<filename>')
def download_comparison_excel(filename):
    """Download the generated Excel comparison file"""
    return send_from_directory(os.path.join(os.getcwd(), 'exports'), filename, as_attachment=True)
