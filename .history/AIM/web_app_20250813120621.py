...
def compare():
    ...
    try:
        # Use THE EXACT SAME CODE as view_data page
        all_data = db_manager.get_all_data()
        
        print(f"Compare page: Using view_data method, found {len(all_data)} records")
        
        # Pass the same data format as view_data but rename for template compatibility
        processed_records = all_data
        
        print("Compare page records:")
        for record in processed_records:
            print(f"  ID: {record.get('id')}, Name: {record.get('name')}, Status: {record.get('status')}")
...
if __name__ == '__main__':
    ...
    # Debug: Test exact view_data method for compare page
