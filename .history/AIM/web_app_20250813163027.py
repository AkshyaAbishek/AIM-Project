...
def process_data_api():
    ...
    try:
        ...
        if result['success']:
            ...
            record = db_manager.get_data_by_id(result['record_id'])
            if record:
                # Process with AIM processor
                    ...
                    with sqlite3.connect(db_manager.db_path) as conn:
                        ...
                        cursor.execute("""
                            ...
                            SET validation_status = 'processed',
                                calculator_path = ?
                            WHERE id = ?
                        """, (f"/calculators/{product_type}/standard.json", result['record_id']))
                        conn.commit()
                except Exception as e:
                    print(f"Error updating record status: {e}")
                ...
                return jsonify({'success': False, 'message': 'Failed to retrieve stored record'})
        else:
