...
class WebDatabaseManager:
    ...
    def load_calculator_configuration(self, calculator_path: str, product_type: str) -> Dict:
        """Load calculator configuration with enhanced field definitions"""
        # Enhanced calculator templates with formulas and date calculations
        calculator_templates = {
            'life': {
...
def apply_calculator_formula(formula, source_data, processed_data):
    """Apply calculator formula with built-in logic"""
    try:
        ...
        for field_name, value in {**source_data, **processed_data}.items():
...
def apply_date_based_calculations(data, calculation_rules):
    """Apply date-based calculations based on effective dates"""
    ...
    try:
        ...
        # Apply date-based rules
        for rule_name, rule_config in calculation_rules.items():
            if rule_config.get('type') == 'date_based':
                data[rule_name] = calculate_date_based_value(effective_date, rule_config, data)
