...
def compare():
    ...
    print("=== COMPARE PAGE - COPYING VIEW DATA LOGIC ===")
    
    # EXACT COPY of view_data route logic
    all_data = db_manager.get_all_data()
    print(f"get_all_data() returned: {len(all_data)} records")
    
    # Use the same variable name as view_data for consistency
    processed_records = all_data
    
    print(f"Setting processed_records to: {len(processed_records)} records")
