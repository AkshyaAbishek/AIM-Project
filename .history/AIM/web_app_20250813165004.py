...
def process_data_api():
    ...
    try:
        ...
        try:
            actual_data = json.loads(json_data_str) if json_data_str else {}
        ...
        print(f"DEBUG: About to call store_data with actual_data type: {type(actual_data)}")
        
        # Store data in database (matching the working upload route exactly)
        result = db_manager.store_data(
            name=name,
            product_type=product_type,
            json_data=actual_data,  # Pass the parsed dict
            session_id=session['session_id'],
            file_source="api_manual_input"  # Add the missing file_source parameter
        )
        
        print(f"DEBUG: store_data result: {result}")
        print(f"DEBUG: result type: {type(result)}")
        
        if result['success']:
            # Process the actual parsed data with AIM processor
            processing_result = aim_processor.process_fast_ui_input(
                actual_data, 
                product_type
            )
                ...
                with sqlite3.connect(db_manager.db_path) as conn:
                    ...
                    cursor.execute("""
                        ...
                        SET validation_status = 'processed',
                            calculator_path = ?
                        WHERE id = ?
                    """, (calculator_path or f"/calculators/{product_type}/standard.json", result['record_id']))
                    conn.commit()
            except Exception as e:
                print(f"Error updating record status: {e}")
            ...
            return jsonify({
                ...
                'redirect_url': f'/process/{result["record_id"]}'
            })
        else:
