...
@app.route('/compare')
def compare():
    """Data comparison page - EXACT COPY OF VIEW_DATA LOGIC"""
    print("=== COMPARE PAGE DEBUG ===")
    
    try:
        # EXACT COPY of view_data route logic - line by line
        ...
        print(f"get_all_data() returned: {type(all_data)} with {len(all_data)} records")
        
        # Calculate statistics for the view (same as view_data)
        stats = {
        ...
        # Use exact same variable name as view_data: 'data'
        # But also provide it as processed_records for template compatibility
        processed_records = all_data
        
        print(f"Final processed_records: {len(processed_records)} records")
        if processed_records:
            print("Sample records from compare route:")
            for i, record in enumerate(processed_records[:3]):
                record_dict = dict(record) if hasattr(record, 'keys') else record
                print(f"  {i+1}. ID={record_dict.get('id')}, Name='{record_dict.get('name')}', Product='{record_dict.get('product_type')}'")
        else:
            ...
            try:
                with sqlite3.connect(db_manager.db_path) as conn:
                    ...
                    print(f"Direct SQL count: {count} records in user_data table")
                    if count > 0:
                        cursor.execute("SELECT id, name, product_type FROM user_data LIMIT 3")
                        raw_records = cursor.fetchall()
                        print(f"Direct SQL sample: {raw_records}")
            except Exception as sql_error:
                print(f"SQL diagnostic error: {sql_error}")
        ...
        traceback.print_exc()
        processed_records = []
        stats = {'total_records': 0, 'avg_quality': 0, 'last_updated': 'Error'}
