"""
DEPRECATED: This is the old web_app.py file - DO NOT USE!

Please use the new organized structure:
- Use launch_aim.py to start the application
- The actual web app is now located at: core/web/web_app.py
- This file is kept for reference only

To start the application:
1. Run: python launch_aim.py
2. This will automatically use core/web/web_app.py with correct paths
"""
...
def main():
    print("=" * 60)
    print("⚠️  WARNING: You are using the DEPRECATED web_app.py file!")
    print("=" * 60)
    print()
    print("After reorganizing your project, please use:")
    print("1. python launch_aim.py  (to start the application)")
    print("2. Edit files in core/web/web_app.py (for modifications)")
    print()
    print("The correct web application is located at:")
    print("   core/web/web_app.py")
    print()
    print("This file has the updated paths for:")
    print("   - Database: database/aim_data.db")
    print("   - Uploads: runtime/uploads/")
    print("   - Exports: runtime/exports/")
    print()
    print("=" * 60)
    
    # Redirect to the correct launcher
    launcher_path = os.path.join(os.path.dirname(__file__), 'launch_aim.py')
    if os.path.exists(launcher_path):
        print("🚀 Automatically launching the correct application...")
        import subprocess
        subprocess.run([sys.executable, launcher_path])
    else:
        print("❌ launch_aim.py not found!")
...
class WebDatabaseManager:
    ...
    def init_database(self):
        ...
        try:
            ...
            columns_to_add = [
                ("name", "TEXT DEFAULT 'Unnamed Record'"),  # Add missing name column
                ...
                ("created_date", "TIMESTAMP DEFAULT CURRENT_TIMESTAMP")  # Add missing created_date column
            ]
    ...
    def get_all_data(self, session_id: Optional[str] = None) -> list:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                data = []
                for row in rows:
                    # Convert SQLite Row to dict and ensure all fields exist
                    record = {}
                    record['id'] = row['id'] if row['id'] is not None else 0
                    record['name'] = row['name'] if row['name'] is not None else 'Unnamed'
                    record['product_type'] = row['product_type'] if row['product_type'] is not None else 'unknown'
                    record['created_date'] = row['created_date'] if row['created_date'] is not None else ''
                    record['validation_status'] = row['validation_status'] if row['validation_status'] is not None else 'processed'
                    record['quality_score'] = row['quality_score'] if row['quality_score'] is not None else 0.0
                    record['file_source'] = row['file_source'] if row['file_source'] is not None else 'Manual Entry'
                    record['data_hash'] = row['data_hash'] if row['data_hash'] is not None else ''
                    
                    # Add additional computed fields
                    record['source_file'] = record['file_source']
                    record['status'] = record['validation_status']  # Keep both for compatibility
...
def process_data_api():
    """API endpoint to process data from the upload form"""
    print("=== STARTING process_data_api ===")
    ...
    try:
        ...
        # Extract required fields with debugging
        name = form_data.get('name', 'Unnamed Entry')
        print(f"DEBUG: name = {name}")
        
        product_type = form_data.get('product_type', 'life')
        print(f"DEBUG: product_type = {product_type}")
        
        calculator_path = form_data.get('calculator_path', '')
        print(f"DEBUG: calculator_path = {calculator_path}")
        
        json_data_str = form_data.get('json_data', '{}')
        print(f"DEBUG: json_data_str = {json_data_str}")
        print(f"DEBUG: json_data_str type = {type(json_data_str)}")
        
        print("Step 3: Parsing JSON data...")
        # Parse the JSON data string to actual data for processing
        try:
            if isinstance(json_data_str, str):
                if json_data_str.strip():
                    actual_data = json.loads(json_data_str)
                    print(f"DEBUG: Successfully parsed JSON string")
                else:
                    actual_data = {}
                    print(f"DEBUG: Empty string, using empty dict")
            else:
                actual_data = json_data_str  # Already parsed
                print(f"DEBUG: Data already parsed")
            ...
            return jsonify({'success': False, 'message': f'Invalid JSON data format: {str(e)}'})
        
        print("Step 4: Setting session...")
        # Generate session ID if not exists
        if 'session_id' not in session:
            session['session_id'] = datetime.now().strftime('%Y%m%d_%H%M%S')
        print(f"DEBUG: session_id = {session.get('session_id')}")
        
        print("Step 5: Creating hash...")
        # Create data hash and store directly to avoid store_data issues
        data_str = json.dumps(actual_data, sort_keys=True)
        data_hash = hashlib.md5(data_str.encode()).hexdigest()
        print(f"DEBUG: data_hash = {data_hash}")
        
        print("Step 6: Database operations...")
        # Store directly in database to bypass any store_data method issues
        with sqlite3.connect(db_manager.db_path) as conn:
            cursor = conn.cursor()
            ...
            if existing:
                print(f"DEBUG: Found duplicate with ID {existing[0]}")
                return jsonify({
                    'success': False, 
                    'message': f'Data already exists (ID: {existing[0]}, Name: {existing[1]})'
                })
            ...
            cursor.execute("""
                INSERT INTO user_data 
                (name, product_type, data_hash, json_data, created_date, 
                 validation_status, quality_score, file_source, session_id, calculator_path)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (
                name, product_type, data_hash, data_str,
                datetime.now().isoformat(), 'processed', 95.0,
                'api_manual_input', session['session_id'], calculator_path
            ))
            
            record_id = cursor.lastrowid
            conn.commit()
            print(f"DEBUG: Successfully stored record with ID: {record_id}")
        
        print("Step 7: Processing with AIM processor...")
        # Process with AIM processor
        processing_result = aim_processor.process_fast_ui_input(actual_data, product_type)
        print(f"DEBUG: processing_result = {processing_result}")
        ...
        response = {
            ...
            'redirect_url': f'/process/{record_id}'
        }
        print(f"DEBUG: Final response = {response}")
        return jsonify(response)
            
    except Exception as e:
        print(f"ERROR in process_data_api: {e}")
