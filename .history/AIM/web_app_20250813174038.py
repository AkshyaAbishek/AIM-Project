...
# Add src directory to path for imports - Updated for root directory location
...
class WebDatabaseManager:
    ...
    def store_data(self, name: str, product_type: str, json_data: Dict, 
        ...
        try:
            print(f"STORE_DATA: Starting data storage for {name} ({product_type})")
            data_str = json.dumps(json_data, sort_keys=True)
            data_hash = hashlib.md5(data_str.encode()).hexdigest()
            print(f"STORE_DATA: Generated hash {data_hash}")
            ...
            with sqlite3.connect(self.db_path) as conn:
                ...
                if existing:
                    print(f"STORE_DATA: Duplicate found - ID: {existing[0]}")
                    return {
                        'success': False,
                        'error': 'duplicate',
                ...
                print(f"STORE_DATA: Calculating quality score...")
                quality_score = self.calculate_quality_score(json_data)
                print(f"STORE_DATA: Quality score: {quality_score}")
                
                print(f"STORE_DATA: Inserting into database...")
                cursor.execute("""
                ...
                print(f"STORE_DATA: Successfully stored with ID: {record_id}")
            ...
            error_msg = f"STORE_DATA ERROR: {str(e)}"
            print(error_msg)
            import traceback
            traceback.print_exc()
            return {
...
def create_excel_template(product_type, sample_data=None):
    """Create an Excel template for data mapping"""
    try:
        import openpyxl
        ...
        wb = openpyxl.Workbook()
        ws = wb.active
        ws.title = f"{product_type.title()} Template"
        ...
        # Sample fields based on product type
        if product_type.lower() == 'annuity':
            sample_fields = [
                ('premium_amount', '10000', 'number', 'Yes', 'Premium amount'),
                ('age', '65', 'number', 'Yes', 'Age of annuitant'),
                ('gender', 'M', 'text', 'Yes', 'Gender (M/F)'),
                ('product_option', 'Standard', 'text', 'Yes', 'Product option'),
            ]
        elif product_type.lower() == 'life_insurance':
            sample_fields = [
                ('sum_assured', '100000', 'number', 'Yes', 'Sum assured amount'),
                ('age', '35', 'number', 'Yes', 'Age of life assured'),
                ('gender', 'F', 'text', 'Yes', 'Gender (M/F)'),
                ('smoker_status', 'Non-smoker', 'text', 'Yes', 'Smoking status'),
            ]
        else:
            sample_fields = [
                ('field1', 'value1', 'text', 'Yes', 'Sample field 1'),
                ('field2', 'value2', 'number', 'No', 'Sample field 2'),
            ]
        ...
        # Add sample data
        for row, (field, value, type_, required, desc) in enumerate(sample_fields, 2):
            ws.cell(row=row, column=1, value=field)
            ws.cell(row=row, column=2, value=value)
            ws.cell(row=row, column=3, value=type_)
            ws.cell(row=row, column=4, value=required)
        ...
        # Save file
        filename = os.path.join(app.config['EXPORT_FOLDER'], f'template_{product_type}_{datetime.now().strftime("%Y%m%d_%H%M%S")}.xlsx')
        wb.save(filename)
        ...
        filename = os.path.join(app.config['EXPORT_FOLDER'], f'template_{product_type}_{datetime.now().strftime("%Y%m%d_%H%M%S")}.txt')
        with open(filename, 'w') as f:
            f.write(f"Excel template for {product_type}\n")
            f.write("Please install openpyxl for Excel template generation\n")
        return filename
...
@app.route('/debug/test-processor')
def test_processor():
    """Debug endpoint to test AIM processor"""
    try:
        ...
        print("Testing AIM Processor...")
        result = aim_processor.process_fast_ui_input(test_data, "annuity")
        print(f"Processor result: {result}")
        
        return jsonify({
            "success": True,
            "processor_type": type(aim_processor).__name__,
            "result": result,
            "test_data": test_data
        })
...
def system_status():
    """Debug endpoint to check system status after reorganization"""
    try:
        status = {
            "database": {
            ...
            "directories": {
                ...
                "exports": {
                    ...
                    "exists": os.path.exists(app.config['EXPORT_FOLDER']),
                    "writable": os.access(app.config['EXPORT_FOLDER'], os.W_OK) if os.path.exists(app.config['EXPORT_FOLDER']) else False
                }
            },
            "processor": {
                "type": type(aim_processor).__name__,
                "is_fallback": type(aim_processor).__name__ == "AIMProcessor" and not hasattr(aim_processor, 'config_manager')
            }
        }
        ...
        try:
            with sqlite3.connect(db_manager.db_path) as conn:
                cursor = conn.cursor()
                cursor.execute("SELECT COUNT(*) FROM user_data")
                status["database"]["record_count"] = cursor.fetchone()[0]
...
def debug_template_paths():
    """Debug endpoint to check Flask template and static paths"""
    return jsonify({
        ...
        "upload_template_exists": os.path.exists(os.path.join(app.template_folder, 'upload.html')),
        "available_templates": os.listdir(app.template_folder) if os.path.exists(app.template_folder) else []
    })
