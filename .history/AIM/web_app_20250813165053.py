...
def process_data_api():
    ...
    try:
        ...
        if isinstance(json_data_str, str) and json_data_str.strip():
            try:
        ...
        # Create a simple data hash for duplicate checking
        data_str = json.dumps(actual_data, sort_keys=True)
        data_hash = hashlib.md5(data_str.encode()).hexdigest()
        
        # Store directly in database without using the complex store_data method
        try:
            with sqlite3.connect(db_manager.db_path) as conn:
                cursor = conn.cursor()
                ...
                if existing:
                    return jsonify({
                        'success': False, 
                        'message': f'Data already exists (ID: {existing[0]})'
                    })
                ...
                cursor.execute("""
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                """, (
                    name, product_type, data_hash, data_str, 'processed',
                    datetime.now().isoformat(), 95.0, 
                    session['session_id'], "api_manual_input"
                ))
