...
def compare():
    ...
    # Get processed records for comparison - use same method as view-data page
    try:
        # Get all data using the same method as view_data page
        all_data = db_manager.get_all_data()
        
        print(f"Compare page: Found {len(all_data)} records from view_data source")
        
        # Show what statuses we have
        if all_data:
            statuses = [record.get('status', 'unknown') for record in all_data]
            print(f"Record statuses: {set(statuses)}")
        
        # If no data exists, add some test data
        if not all_data:
            print("No data found, adding test records...")
            ...
            # Re-fetch data
            all_data = db_manager.get_all_data()
            print(f"After adding test data: Found {len(all_data)} records")
        
        # Convert to format needed for compare page (with json_data field)
        processed_records = []
        with sqlite3.connect(db_manager.db_path) as conn:
            conn.row_factory = sqlite3.Row
            cursor = conn.cursor()
            ...
            for record in all_data:
                ...
                row = cursor.fetchone()
                if row:
                    processed_records.append(dict(row))
...
if __name__ == '__main__':
    ...
    # Debug: Force restart to test compare page data source changes
