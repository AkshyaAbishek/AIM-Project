...
Ensure all string values are in quotes and numbers are without quotes."""
    ...
    def show_bulk_preview_dialog(self, processed_data, error_count, upload_path):
        ...
        # Save options frame
        save_frame = tk.LabelFrame(content_frame, text="💾 Save Options", 
                                 font=("Arial", 11, "bold"), bg="#fff3e0", fg="#f57c00")
        save_frame.pack(fill="x", pady=(0, 15))
        
        options_content = tk.Frame(save_frame, bg="#fff3e0")
        options_content.pack(pady=15, padx=15)
        
        tk.Label(options_content, text="Choose how to handle the processed data:", 
                font=("Segoe UI", 10, "bold"), fg="#f57c00", bg="#fff3e0").pack(anchor="w", pady=(0, 10))
        
        save_option = tk.StringVar(value="save_all")
        
        tk.Radiobutton(options_content, text=f"� Save all {len(processed_data)} records to database", 
                      variable=save_option, value="save_all", 
                      bg="#fff3e0", font=("Segoe UI", 10)).pack(anchor="w", pady=2)
        
        tk.Radiobutton(options_content, text="👀 Preview only (don't save to database)", 
                      variable=save_option, value="preview_only", 
                      bg="#fff3e0", font=("Segoe UI", 10)).pack(anchor="w", pady=2)
        ...
        def execute_save():
            if save_option.get() == "save_all":
                self.save_bulk_data_to_database(processed_data, dialog)
            else:
                dialog.destroy()
                self.log_result("👀 Bulk data preview completed - no data saved")
                messagebox.showinfo("Preview Complete", 
                                   f"Preview completed!\n\n"
                                   f"Records reviewed: {len(processed_data)}\n"
                                   f"No data was saved to the database.")
        
        tk.Button(button_frame, text="✅ Execute", command=execute_save,
                 bg="#27ae60", fg="white", font=("Arial", 12), relief="raised", bd=2).pack(side="left", padx=10)
        
        tk.Button(button_frame, text="❌ Cancel", command=dialog.destroy,
                 bg="#e74c3c", fg="white", font=("Arial", 12), relief="raised", bd=2).pack(side="left", padx=10)
