...
class WebDatabaseManager:
    ...
    def perform_enhanced_data_comparison(self, source_data: Dict, calculator_data: Dict) -> Dict:
        """Enhanced comparison with True/False matching and detailed analysis"""
        ...
        for field_name, field_config in calculator_fields.items():
            ...
            if source_value is not None and source_value != '':
                if calculator_value is not None and calculator_value != '':
                    # Type-specific comparison
                    if field_type in ['number', 'currency']:
                        try:
                            source_num = float(str(source_value).replace('$', '').replace(',', ''))
                            calc_num = float(str(calculator_value).replace('$', '').replace(',', ''))
                            values_match = abs(source_num - calc_num) < 0.01  # Allow for small rounding differences
                        except (ValueError, TypeError):
                            values_match = str(source_value).strip().lower() == str(calculator_value).strip().lower()
                    elif field_type == 'date':
                        # Normalize date formats for comparison
                        source_date = str(source_value).replace('/', '-')
                        calc_date = str(calculator_value).replace('/', '-')
                        values_match = source_date == calc_date
                    elif field_type == 'boolean':
                        source_bool = str(source_value).lower() in ['true', 'yes', '1', 'y']
                        calc_bool = str(calculator_value).lower() in ['true', 'yes', '1', 'y']
                        values_match = source_bool == calc_bool
                    else:
                        # String comparison (case-insensitive)
                        values_match = str(source_value).strip().lower() == str(calculator_value).strip().lower()
                    
                    comparison_status = 'match' if values_match else 'mismatch'
                    if values_match:
                        matching_fields += 1
                    else:
                        mismatched_fields += 1
        ...
        return {
            'stats': {
                ...
                'filled_fields': filled_fields,
                'completion_percentage': round((matching_fields / total_fields) * 100, 2) if total_fields > 0 else 0,
                'accuracy_percentage': round((matching_fields / (matching_fields + mismatched_fields)) * 100, 2) if (matching_fields + mismatched_fields) > 0 else 0
            },
            'fields': comparison_fields
        }
...
def compare():
    ...
    # Get all processed records for comparison (not just by session)
    processed_records = []
    with sqlite3.connect(db_manager.db_path) as conn:
        conn.row_factory = sqlite3.Row
        cursor = conn.cursor()
        cursor.execute("""
            SELECT id, name, product_type, created_date, calculator_path, quality_score
            FROM user_data 
            WHERE json_data IS NOT NULL AND json_data != ''
...
def api_compare_data():
    """API endpoint to compare source data with calculator template and generate Excel"""
    try:
        data = request.get_json()
        ...
        # Load calculator configuration
        calculator_config = db_manager.load_calculator_configuration(calculator_path, record.get('product_type', ''))
        
        # Perform comparison with enhanced True/False matching
        comparison_result = db_manager.perform_enhanced_data_comparison(source_data, calculator_config)
        
        # Generate Excel file with comparison results
        excel_filename = generate_comparison_excel(comparison_result, record, calculator_path)
        ...
        return jsonify({
            ...
            'download_url': f'/api/download-comparison/{excel_filename}'
        })
...
def api_save_filled_data():
    """API endpoint to save filled data as a new record"""
    try:
        data = request.get_json()
        ...
        if not name or not product_type or not calculator_path or not filled_data:
            return jsonify({
                'success': False,
                'message': 'Name, product type, calculator path, and filled data are required'
            }), 400
        ...
        result = db_manager.store_processed_data(
            name=name,
            product_type=product_type,
            source_data={},  # Empty as we're using filled data
            processed_data=filled_data,
            calculator_path=calculator_path,
            field_mapping={},  # No mapping needed for filled data
            session_id=session.get('session_id', os.urandom(16).hex()),
            file_source='compare_filled',
        ...
        if result['success']:
            return jsonify({
                'success': True,
                'message': f'Filled data saved successfully! Quality Score: {result["quality_score"]}%',
                'record_id': result['record_id'],
                'quality_score': result['quality_score']
            })
        else:
        ...
        return jsonify({
            'success': False,
            'message': f'Error saving filled data: {str(e)}'
        }), 500
        ...
        if calculation_rules:
            for rule_name, rule_config in calculation_rules.items():
                if rule_name == 'premium_calculation' and 'face_amount' in data and 'applicant_age' in data:
                    ...
                    # Calculate account value based on contract year
                    contract_year = int(data.get('contract_year', 1))
                    account_value = initial_premium * math.pow(1 + guaranteed_rate, contract_year - 1)
...
def generate_comparison_excel(comparison_result, record, calculator_path):
    """Generate Excel file with comparison results including True/False matching"""
    try:
        import openpyxl
    ...
    wb = openpyxl.Workbook()
    ws = wb.active
    ws.title = "Data Comparison Results"
    ...
    ws['A1'] = 'AIM Data Comparison Report'
    ws['A1'].font = Font(bold=True, size=16)
    ws.merge_cells('A1:G1')
    
    ws['A3'] = 'Source Record:'
    ws['B3'] = record.get('name', 'Unknown')
    ws['A4'] = 'Product Type:'
    ws['B4'] = record.get('product_type', 'Unknown')
    ws['A5'] = 'Calculator:'
    ws['B5'] = calculator_path
    ws['A6'] = 'Comparison Date:'
    ws['B6'] = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    
    # Summary statistics
    stats = comparison_result['stats']
    ws['D3'] = 'Total Fields:'
    ws['E3'] = stats['total_fields']
    ws['D4'] = 'Matching:'
    ws['E4'] = stats['matching_fields']
    ws['D5'] = 'Mismatched:'
    ws['E5'] = stats['mismatched_fields']
    ws['D6'] = 'Missing:'
    ws['E6'] = stats['missing_fields']
    ws['D7'] = 'Accuracy:'
    ws['E7'] = f"{stats['accuracy_percentage']:.1f}%"
    ...
    # Data rows
    for row_num, field in enumerate(comparison_result['fields'], start_row + 1):
        # Field Name
        ws.cell(row=row_num, column=1, value=field['field_name']).border = thin_border
        
        # Source Value
        source_val = field['source_value'] if field['source_value'] is not None else 'N/A'
        ws.cell(row=row_num, column=2, value=str(source_val)).border = thin_border
        
        # Calculator Value
        calc_val = field['calculator_value'] if field['calculator_value'] is not None else 'N/A'
        ws.cell(row=row_num, column=3, value=str(calc_val)).border = thin_border
        
        # Values Match (True/False)
        match_cell = ws.cell(row=row_num, column=4, value=field['values_match'])
        match_cell.border = thin_border
        if field['values_match']:
            match_cell.fill = PatternFill(start_color='90EE90', end_color='90EE90', fill_type='solid')
        else:
            match_cell.fill = PatternFill(start_color='FFB6C1', end_color='FFB6C1', fill_type='solid')
        
        # Status
        status_cell = ws.cell(row=row_num, column=5, value=field['status'].title())
        status_cell.border = thin_border
        
        # Status color coding
        if field['status'] == 'match':
            status_cell.fill = PatternFill(start_color='90EE90', end_color='90EE90', fill_type='solid')
        elif field['status'] == 'mismatch':
        ...
        elif field['status'] == 'can_fill':
            status_cell.fill = PatternFill(start_color='E0E0FF', end_color='E0E0FF', fill_type='solid')
        
        # Data Type
        ws.cell(row=row_num, column=6, value=field['data_type']).border = thin_border
        
        # Required
        ws.cell(row=row_num, column=7, value='Yes' if field['required'] else 'No').border = thin_border
    ...
    # Save file
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    filename = f'comparison_results_{record.get("id", "unknown")}_{timestamp}.xlsx'
...
@app.route('/api/download-comparison/<filename>')
def download_comparison_excel(filename):
