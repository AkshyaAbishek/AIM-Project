...
def compare():
    ...
    print("=== COMPARE PAGE - SIMPLIFIED DEBUG ===")
    
    # EXACT same logic as view_data route
    all_data = db_manager.get_all_data()
    print(f"Retrieved {len(all_data)} records from database")
    
    # For template: use processed_records (compare template expects this)
    processed_records = all_data
    
    if processed_records:
        print(f"✅ SUCCESS: {len(processed_records)} records found")
        print(f"Sample: {dict(processed_records[0])}")
    else:
        print("❌ PROBLEM: No records found")
