...
def create_excel_template(product_type: str, sample_data: Dict = None) -> str:
    """Create an Excel template based on product type"""
    try:
        # Create a new Excel workbook
        # Create sample templates based on product type
        wb = pd.DataFrame()
        ...
        if sample_data:
            # If sample data is provided, add it as a row
            wb = pd.concat([wb, pd.DataFrame([sample_data])], ignore_index=True)
                    ("processed_data", "TEXT"),
                    ("field_mappings", "TEXT"),
                ...
                for column_name, column_type in columns_to_add:
                    try:
                        ...
                        cursor.execute(f"SELECT count(*) FROM pragma_table_info('user_data') WHERE name=?", (column_name,))
                        if cursor.fetchone()[0] == 0:
                            print(f"Adding column {column_name} to user_data table")
                            conn.execute(f"ALTER TABLE user_data ADD COLUMN {column_name} {column_type}")
                    except sqlite3.OperationalError as e:
                        print(f"Note: Column {column_name} already exists or error: {e}")
                
                # Add session_id to processing_logs if it doesn't exist
                cursor = conn.cursor()
                cursor.execute("SELECT count(*) FROM pragma_table_info('processing_logs') WHERE name='session_id'")
                if cursor.fetchone()[0] == 0:
                    print("Adding session_id column to processing_logs")
    ...
    try:
        with sqlite3.connect(db_manager.db_path) as conn:
            ...
            return jsonify({
                'success': True,
                'count': count,
                'records': [{'id': r[0], 'name': r[1], 'status': r[2], 'type': r[3]} for r in records],
                'schema': [{'name': c[1], 'type': c[2]} for c in columns],
                'database_path': db_manager.db_path,
                'file_exists': os.path.exists(db_manager.db_path)
            })
    except Exception as e:
        ...
        return jsonify({
            'success': False,
            'error': str(e),
            'database_path': db_manager.db_path if hasattr(db_manager, 'db_path') else 'unknown',
            'file_exists': os.path.exists(db_manager.db_path) if hasattr(db_manager, 'db_path') else False
        })
